...
func TestStackSlicePush(t *testing.T) {
	tests := []struct{
		stack 	  	  []int
		itemToPush 	  int
		updatedStack  []int
	...
	for _, test := range tests {
		if got := Push(test.stack, test.itemToPush); !reflect.DeepEqual(got, test.updatedStack) {
			t.Errorf("expected %v, got %v", test.updatedStack, got)
		}
	}
}

func TestStackSlicePop(t *testing.T) {
	tests := []struct{
		stack 	  	  []int
		updatedStack  []int
	}{
	...
	for _, test := range tests {
		if got := Pop(test.stack); !reflect.DeepEqual(got, test.updatedStack) {
			t.Errorf("expected %v, got %v", test.updatedStack, got)
		}
	}
}
